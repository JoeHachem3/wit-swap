...
describe('WITSwap', function () {
  ...
  it('#createPool 4: should reject', async () => {
    await token1.transfer(token2.address, toWei(90));
    return expect(
  ...
  it('#createPool 5: should reject', async () => {
    await token2.transfer(token1.address, toWei(90));
    return expect(
      contract.createPool(token1.address, toWei(20), token2.address, toWei(20))
    ).to.eventually.be.rejected;
  });

  it('#createPool 6: should reject', async () => {
  ...
  it('#createPool 7: should emit #PoolCreate, set #pools[token1.address][token2.address] and #pools[token2.address][token1.address], and transfer the right amount to the right addresses', async () => {
    ...
    expect(await token1.balanceOf(owner.address)).to.equal(toWei(70));
    expect(await token2.balanceOf(owner.address)).to.equal(toWei(70));
  });

  it('#getTokenReturnedAmount 1: should reject', () => {
    return expect(
  ...
  it('#sellToken 2: should return a valid amount', async () => {
    await token1.approve(contract.address, toWei(30));
    await token2.approve(contract.address, toWei(30));
    await contract.createPool(
      token1.address,
  ...
  it('#sellToken 3: should emit Swapped', async () => {
    await token1.approve(contract.address, toWei(30));
    await token2.approve(contract.address, toWei(30));
    await contract.createPool(
      token1.address,
  ...
  it('#buyToken 2: should return a valid amount', async () => {
    await token1.approve(contract.address, toWei(30));
    await token2.approve(contract.address, toWei(30));
    await contract.createPool(
      token1.address,
  ...
  it('#buyToken 3: should emit Swapped', async () => {
    await token1.approve(contract.address, toWei(30));
    await token2.approve(contract.address, toWei(30));
    await contract.createPool(
      token1.address,
  ...
  it('#withdrawLiquidity 1: should reject', () => {
    return expect(
      contract.withdrawLiquidity(token1.address, token2.address, toWei(1))
    ).to.eventually.rejected;
  });

  it('#withdrawLiquidity 2: should emit LiquidityWithdrawn', async () => {
    await token1.approve(contract.address, toWei(30));
    await token2.approve(contract.address, toWei(30));
    await contract.createPool(
      token1.address,
